...
export default function NeslabsLanding() {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white text-black dark:bg-black dark:text-white">
      ...
      <header className="sticky top-0 z-40 border-b border-black/10 bg-white/90 backdrop-blur dark:border-white/10 dark:bg-black/60">
        <nav className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          <button onClick={() => setPage('home')} className="flex items-center gap-2">
            <span className="text-2xl font-bold" style={{ color: "#fff" }}>נֵס</span>
            <span className="text-lg tracking-tight font-medium">Neslabs</span>
          </button>
          <div className="flex items-center gap-6 text-sm font-medium">
            <button onClick={() => setPage('projects')} className={`hover:underline ${page==='projects' ? 'underline' : ''}`}>Projects</button>
            <button onClick={() => setPage('why')} className={`hover:underline ${page==='why' ? 'underline' : ''}`}>Why Neslabs</button>
            <a
            ...
            {/* <Button
              variant="outline"
              className="border border-black text-black bg-transparent px-4 py-2 text-sm hover:bg-neutral-100 dark:border-white dark:text-white dark:hover:bg-neutral-900"
      ...
      {page === 'home' && <HomeSection onGoProjects={() => setPage('projects')} onGoContact={() => setPage('contact')} />}
...
>
  Build with us
            ...
            {/* <Button
              variant="outline"
              className="px-6 py-3 text-sm border border-black text-black hover:bg-gradient-to-r hover:from-neutral-200 hover:to-neutral-300 dark:border-white dark:text-white dark:hover:from-neutral-700 dark:hover:to-neutral-800"
      ...
      <section className="py-20 px-6 border-t border-black/10 dark:border-white/10">
        <div className="max-w-6xl mx-auto">
          ...
          <p className="mt-2 text-black/70 dark:text-white/70">A focused set of tools that form a dependable on‑chain stack.</p>
          <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            <ProjectCard name="Mothrbox" desc="Encrypted storage & infrastructure primitives. Dev‑friendly tooling for secure blob storage and retrieval." />
            <ProjectCard name="Suiref" desc="On‑chain referral system with attendance verification, NFT rewards, and leaderboards to grow real communities." />
            {/* <ProjectCard name="Iruka" desc="CLI & SDK for automation around storage and deployments. Scriptable workflows for builders and ops." />
            <ProjectCard name="Suifun" desc="Launchpad with bonding‑curve mechanics and exchange connectors. Anti‑bot rails for fair token launches." />
            <ProjectCard name="Suihort" desc="Cohort & workshop management with credentials. ZK login, module tracks, and organizer tooling." /> */}
          </div>
        </div>
      </section>
      ...
      <section className="py-20 px-6 border-t border-black/10 dark:border-white/10 bg-gradient-to-r from-neutral-100 to-neutral-300 dark:from-neutral-800 dark:to-black">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="mt-3 text-black/70 dark:text-white/70">Pilots, integrations, or collaborations—let’s ship something real.</p>
          <div className="mt-6">
            {/* <a
                href="https://x.com/nes_labs"
                className="px-6 py-3 text-sm bg-black text-white rounded-md hover:bg-gradient-to-r hover:from-black hover:to-neutral-700 dark:bg-white dark:text-black dark:hover:bg-gradient-to-r dark:hover:from-white dark:hover:to-neutral-400"
...
function ProjectsPage() {
  const projects = [
    ...
    // { name: 'Iruka', points: ['CLI + SDK automation', 'Pipelines & deployments', 'DevOps-friendly'] },
    // { name: 'Suifun', points: ['Bonding-curve launchpad', 'DEX connectors', 'Anti-bot protections'] },
    // { name: 'Suihort', points: ['Cohort/workshop tools', 'Credentials & ZK login', 'Organizer dashboard'] },
  ];
  ...
  return (
    <main className="flex-1">
      <section className="px-6 py-16 text-center bg-gradient-to-b from-neutral-50 to-neutral-200 dark:from-neutral-900 dark:to-black">
        ...
        <p className="mt-3 text-black/70 dark:text-white/70">An integrated on‑chain stack for builders and enterprises.</p>
      </section>
        ...
        <div className="max-w-2xl mx-auto">
          <form className="space-y-4">
            <div>
              ...
              <input className="mt-1 w-full rounded-md border border-black/20 bg-transparent p-3 dark:border-white/20" placeholder="Your name"/>
            </div>
            <div>
              <label className="text-sm">Email</label>
              <input type="email" className="mt-1 w-full rounded-md border border-black/20 bg-transparent p-3 dark:border-white/20" placeholder="you@company.com"/>
            </div>
            <div>
              <label className="text-sm">What do you want to build?</label>
              <textarea rows={5} className="mt-1 w-full rounded-md border border-black/20 bg-transparent p-3 dark:border-white/20" placeholder="Briefly describe your project"/>
            </div>
            <div className="flex items-center justify-between">
              <p className="text-xs text-black/60 dark:text-white/60">Or email us: <a className="underline" href="mailto:hello@neslabs.io">hello@neslabs.io</a></p>
              <Button className="px-6 py-2 text-sm bg-black text-white hover:bg-neutral-800 dark:bg-white dark:text-black dark:hover:bg-neutral-200">Send</Button>
            </div>
          </form>
        </div>
      </section>
